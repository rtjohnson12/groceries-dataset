...
def main(interactive):
    ...
    fig.write_image('data/processed/sales_by_retailer.png', width = 1980, height = 1080)
    ...
    fig.write_image('data/processed/sales_through_time.png', width = 1980, height = 1080)
    if interactive: fig.show()

    logger.info('generating `data/processed/normalized_sales_through_time.png`')
    norm_merged_df = merged_df
    norm_merged_df['Sold'] = norm_merged_df.groupby(['RetailerName', 'CategoryName'])['Sold'].apply(lambda x: (x - x.min())/(x.max() - x.min()))
    ...
    fig.update_yaxes(matches = None)
    fig.write_image('data/processed/normalized_sales_through_time.png', width = 1980, height = 1080)
    if interactive: fig.show()
